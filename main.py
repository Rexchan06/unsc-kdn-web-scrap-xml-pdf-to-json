...
from utils.common_utils import calculate_sha256

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

from UNSC.unsc_web_scraper import get_xml_link
...
def run_unsc_sanction_list_process(local_mode: bool = False):
    ...
    try:
        ...
        response = requests.get(xml_link, verify=True)
        ...
        if current_xml_url != last_known_xml_url:
            ...
            if pdf_content_bytes:
                ...
                if not local_mode:
                    ...
                    from utils.local_file_utils import write_local_state_file
...
def lambda_handler(event, context):
    ...
    run_unsc_sanction_list_process(local_mode=False)
    run_kdn_sanction_list_process(local_mode=False)
